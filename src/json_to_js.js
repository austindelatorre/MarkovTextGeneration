var fs = require('fs');
var json_data = fs.readFileSync('dict.json', 'utf8');
 ...
 */
function predict_words(data, seed_key, n_words, constant_key_size, max_key_size) {
    let result = [];

    for (let i = 0; i < n_words; i++) {
        if (i != 0) {
            if (constant_key_size !== true){
                lower_index = Math.max(0, i - max_key_size); // will not go back into negative indices
                result_slice = result.slice(lower_index, i + 1); // not sure about +1
